...
def main_function():
    ...
    def extract_voltage_data(voltage_file, current_file, start_time_str, duration_hours):
        ...
        filtered_voltage_data['DTW_wasserstein_initial_time_cutoff'] = datetime.strptime('2024-07-11 03:15', '%Y-%m-%d %H:%M')
    ...
    '''
    这里给充电电流分成三个阶段，也可以根据实际情况自行选择分段
    存储为First Charge,Second Charge和 Third Charge
    恒压恒流计算用到的是Third Charge
    '''
    ...
    def calculate_dtw(filtered_voltage_data):
        ...
        """
            ...
            filtered_voltage_data (DataFrame): 包含电压数据的DataFrame，必须包含以下列:
                ...
                - 'clique_name': 电池名称
                - 'val': 电压值
                - 'DTW_wasserstein_initial_time_cutoff': DTW和Wasserstein初始时间分割点
                - 'DTW_wasserstein_current_time_cutoff': DTW和Wasserstein当前时间分割点

            返回:
            DataFrame: 包含每节电池DTW距离的DataFrame，列为:
                - 'Battery': 电池名称
                - 'DTW_distance': DTW距离

            详细说明:
            1. 根据初始时间分割点和当前时间分割点，分割数据为初始电压循环曲线和当前电压循环曲线。
            2. 对每节电池，计算初始电压循环曲线和当前电压循环曲线之间的DTW距离。
            3. 返回一个包含每节电池名称及其DTW距离的DataFrame。
            """
    ...
    def calculate_wasserstein_distance(filtered_voltage_data):
        """
            ...
            filtered_voltage_data (DataFrame): 包含电压数据的DataFrame，必须包含以下列:
                ...
                - 'clique_name': 电池名称
                - 'val': 电压值
                - 'DTW_wasserstein_initial_time_cutoff': Wasserstein初始时间分割点
                - 'DTW_wasserstein_current_time_cutoff': Wasserstein当前时间分割点

            返回:
            DataFrame: 包含每节电池Wasserstein距离的DataFrame，列为:
                - 'Battery': 电池名称
                - 'Wasserstein_distance': Wasserstein距离

            详细说明:
            1. 根据初始时间分割点和当前时间分割点，分割数据为初始电压循环曲线和当前电压循环曲线。
            2. 对每节电池，计算初始电压循环曲线和当前电压循环曲线之间的Wasserstein距离。
            3. 返回一个包含每节电池名称及其Wasserstein距离的DataFrame。
            """

        initial_time_cutoff = filtered_voltage_data['DTW_wasserstein_initial_time_cutoff'].iloc[0]
        current_time_cutoff = filtered_voltage_data['DTW_wasserstein_current_time_cutoff'].iloc[0]
        # 分割数据为初始电压循环曲线和当前电压循环曲线
        initial_curve = filtered_voltage_data[filtered_voltage_data['time'] <= initial_time_cutoff]
        current_curve = filtered_voltage_data[filtered_voltage_data['time'] >= current_time_cutoff]
